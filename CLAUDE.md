...
| `dictionary/<family>-x.y.json` | Per-family canonical keys, units, groups, aliases, hints. `ldo` (62 params), `mosfet` (45), `voltage_reference` (41), `op_amp` (59), `dc_dc` (92), `diode` (39), `bjt` (55). |
...
- **Unit enum (closed):** `V A Hz degC ohm F H s W C J S deg dB V/V % ppm ppm/degC ppm/V ppm/A V/degC A/degC degC/W K/W V/us A/us V/sqrtHz A/sqrtHz A2s`. No SI prefixes (a real per-mA/uV rate becomes `ppm/A` or `ohm`, never `ppm/mA`). `S` is siemens (transconductance = A/V), `deg` is phase-margin degrees, `A/sqrtHz` is current-noise density, `H` is henries (recommended/integrated inductance), `A2s` is ampere-squared-seconds (a rectifier's I²t fusing rating). A printed `%/degC` breakdown tempco converts to `ppm/degC` (×10⁴); RF power in `dBm` converts to `W` (`10^((dBm−30)/10)`), both with the printed form kept in `conditionsVerbatim` — neither enters the enum.
...
- **Condition axis `param` vocab:** `T_J T_A T_C T_SP T_L V_IN V_OUT V_EN V_S V_CM V_GS V_DS V_FB V_SW V_CC V_CCX V_SS V_C V_SENSE V_RREF V_R V_CE V_CB V_EB I_OUT I_LOAD I_R I_C I_D I_S I_G I_CC I_F I_T I_PP I_Z I_RR I_B I_E F F_SW DI_DT DUTY C_OUT C_IN C_NR C_FF C_SET C_L C_SS C_RAMP ESR R_L R_S R_F R_G R_T R_ILIM R_TC R_BE A_V ACCURACY HEADROOM RIPPLE BW_LOW BW_HIGH C_OUT_TYPE MODE SWITCH DRIVER N_PS WAVEFORM PACKAGE BOARD DIRECTION` (op-amps add V_CM/R_L/A_V/C_L/R_S; dc_dc adds F_SW/V_C/V_SENSE/V_RREF/I_CC/C_RAMP/R_TC plus note-only SWITCH/DRIVER/N_PS; diodes add I_F/V_R/I_T/I_PP/I_Z/I_RR/T_L and note-only WAVEFORM (pulse descriptor); BJTs add V_CE/V_CB/V_EB/I_B/I_E/R_BE) (extensible; a `unit` is required whenever a numeric `value`/`min`/`max` is present, omitted for note-only axes like `DIRECTION`=source/sink/shunt or `WAVEFORM`=10/1000us). The dimension check in `regression.mjs` enforces `T_*`→degC, `V_*`→V, `I_*`→A, `R_*`→ohm, `F`/`BW_*`→Hz, `C_*`→F; other axes (e.g. `DI_DT`=A/us, `A_V`=dimensionless, `N_PS`=V/V) are unconstrained.
