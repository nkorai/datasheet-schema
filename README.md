...
3. Envelope and dictionaries. The parameter shape is family-agnostic. A family dictionary supplies the canonical keys, units, and vendor aliases, so that `psrr`, `ripple rejection`, and `power supply ripple rejection` map to one key. Eighteen families are included, spanning the active devices — an LDO regulator, a switching (DC-DC) regulator, a discrete power MOSFET, a JFET, a bipolar junction transistor, a diode (rectifier / Schottky / Zener / TVS), a precision voltage reference, an operational amplifier, a comparator, an analog switch, an analog-to-digital converter, and a digital-to-analog converter — the passives an analog board is built from — a capacitor, a resistor, and an inductor — the timing reference that clocks it, a quartz crystal — and the optoelectronic emitters, a light-emitting diode (visible indicator, white lighting, RGB, infrared, or ultraviolet) and a laser diode (Fabry-Perot, DFB, VCSEL, or high-power). All share one measurement envelope, condition model, and provenance record. A pin function is likewise an open uppercase vocabulary, so the same `pinout` structure carries a regulator's IN, OUT, GND, a field-effect transistor's G, D, S, a bipolar transistor's BASE, COLLECTOR, EMITTER, a diode's or LED's ANODE, CATHODE, a laser diode's LD_ANODE, LD_CATHODE with its PD_ANODE, PD_CATHODE monitor, an op-amp's IN_P, IN_N, VS_POS, VS_NEG, a non-polar passive's T1, T2, and a crystal's XTAL1, XTAL2. Adding a family requires only a new dictionary, not a schema change.
